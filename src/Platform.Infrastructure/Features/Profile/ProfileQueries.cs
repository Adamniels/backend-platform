...
public sealed class ProfileQueries(PlatformDbContext db) : IProfileQueries
{
    public async Task<UserProfileDto> GetProfileAsync(CancellationToken cancellationToken = default)
    {
        var p = await db.Profiles.AsNoTracking().SingleAsync(x => x.Id == PlatformProfile.SingletonKey, cancellationToken);
    ...
    {
        var s = await db.UserSettings.AsNoTracking().SingleAsync(x => x.Id == PlatformUserSettings.SingletonKey, cancellationToken);
